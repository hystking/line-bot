...
const createDialogueRequestData = require("../create-dialogue-request-data")
...
function brainstormingTalk(state, action) {
  ...
  if(state.brainstorminReceivingConfirmIdea) {
    if(isYes(text)) {
      return Object.assign({}, state, {
        ...
        brainstormingIdeas: state.brainstormingIdeas.concat([state.brainstormingLastIdea]),
        messages: state.messages.concat([{
  ...
  if(state.brainstormingKeywords.length > 0) {
    ...
    return Object.assign({}, state, {
      dialogues: state.dialogues.concat([{
        requestData: createDialogueRequestData(text),
        to: [from],
      }]),
    })
  }
